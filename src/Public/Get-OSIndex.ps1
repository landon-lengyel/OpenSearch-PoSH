function Get-OSIndex {
    <#
    .SYNOPSIS
        List (cat) all indices.
    ...
    .PARAMETER PrimaryOnly
        Return information only from primary shards.
    ...
    if ($PrimaryOnly -eq $True){
        [Void]$Urlparameter.Append("&pri=true")
    }
    ...
    # Build request
    if ($Index.Count -ge 1){
        $Request = '/_cat/indices/' + [String]$Index + $UrlParameterString
    }
    else {
        $Request = '/_cat/indices' + $UrlParameterString
    }
...
Export-ModuleMember -Function Get-OSIndex
