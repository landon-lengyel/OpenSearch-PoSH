function Get-OSSegment {
    <#
    .SYNOPSIS
        List all Lucene segments.

    .DESCRIPTION
        List all Lucene segments, or all segments that make up a given index.
    ...
    # Build request
    if ($Index.Count -ge 1){
        $Request = '/_cat/segments/' + [String]$Index + $UrlParameterString
    }
    else {
        $Request = '/_cat/segments' + $UrlParameterString
    }
...
Export-ModuleMember -Function Get-OSSegment
